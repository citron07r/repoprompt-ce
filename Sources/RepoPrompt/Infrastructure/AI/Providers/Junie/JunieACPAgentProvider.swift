...
struct JunieACPAgentProvider: ACPAgentProvider {
    ...
    init(
        ...
        launchResolver: JunieACPLaunchResolver = JunieACPLaunchResolver()
    ) {
        self.config = config
    ...
    func normalizeSessionUpdate(
        ...
        JunieACPEventNormalizer.normalize(payload)
    }

    func preferredAuthMethodID(context: ACPAuthenticationContext) -> String? {
        let envTokenKeys = ["JUNIE_API_KEY"]
        ...
        return context.authMethodIDs.first {
            $0.caseInsensitiveCompare("junie_api_key") == .orderedSame
        }
    }
    ...
    func normalizeError(_ error: Error) -> Error {
        ...
        {
            return AIProviderError.invalidConfiguration(detail: "Junie CLI not found. Install it and ensure `junie` is available on PATH.")
        }
        if error is JunieACPLaunchResolutionError || error is ExecutableFileIdentityError {
            return AIProviderError.invalidConfiguration(detail: error.localizedDescription)
        }
        if (error as NSError).domain == NSCocoaErrorDomain {
            return AIProviderError.invalidConfiguration(detail: "Unable to prepare Junie ACP session: \(error.localizedDescription)")
        }
